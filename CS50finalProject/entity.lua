...
function Entity:update(dt)
  ...
  self.gravity = self.gravity + self.weight * 2 *dt
...
function Entity:resolveCollision(e)
    ...
    if self:checkCollision(e) then
      ...
      if self:wasVerticallyAligned(e) then
        if e:is(WallNoBottom) then
          return false
        elseif self.x + self.width/2 < e.x + e.width/2 then
...
--e is other entity to check collision with
...
--needed to account for quads having excessively hitboxes when used with getwidth() and getheight()
function Entity:checkQuadCollision(e, frameWidth, frameHeight)
  return self.x + self.width > e.x
  and self.x < e.x + frameWidth
  and self.y + self.height > e.y
  and self.y < e.y + frameHeight
end
